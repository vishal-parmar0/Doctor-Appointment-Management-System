...
def send_message():
    ...
    appointment_exists = Appointment.query.filter(or_(
        and_(Appointment.patient_id == user_id, Appointment.doctor_id == receiver_id),
        and_(Appointment.patient_id == receiver_id, Appointment.doctor_id == user_id)
    )).first()
...
def mark_read(id):
    ...
    current_user = json.loads(get_jwt_identity())
    user_id = current_user['id']
    msg = Message.query.filter_by(id=id, receiver_id=user_id).first()
...
def mark_all_read():
    ...
    current_user = json.loads(get_jwt_identity())
    user_id = current_user['id']
    db.session.query(Message).filter_by(receiver_id=user_id, is_read=False).update({"is_read": True})
    db.session.commit()
